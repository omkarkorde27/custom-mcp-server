...
async function executeFile(
  filePath: string,
  args: string[] = [],
  interpreter: 'python' | 'node' | 'auto' = 'auto',
...
async function executePackageManager(
  command: 'npm' | 'pip' | 'pip3',
  args: string[] = [],
  workingDirectory?: string,
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "Execute npm or pip package manager commands safely. " +
        ...
        inputSchema: zodToJsonSchema(ExecutePackageManagerArgsSchema) as ToolInput,
      },
    ],
  };
});
  ...
  try {
    ...
    switch (name) {
      ...
      case "edit_file": {
        ...
        const edits: FileEdit[] = parsed.data.edits.map(edit => ({
      ...
      case "execute_package_manager": {
        ...
        const result = await executePackageManager(
          parsed.data.command,
          parsed.data.args,
          parsed.data.workingDirectory,
          parsed.data.timeout
